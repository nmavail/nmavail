...
- Fixed `.ly` domain check logic (status field instead of domain\_name)
...
- Fixed `.ly` domain check logic (use status field instead of domain\_name)
- Fixed AUR and Debian package check logic
...
- Unified timeout configuration (DEFAULT\_TIMEOUT=10s)
